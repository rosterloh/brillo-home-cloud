_envsetup_load() {
  ...
  local bdk="${BDK_PATH:-/home/osterloh/brillo}"
  ...
  local product_path="${PRODUCT_PATH:-homeCloud}"
  ...
  local bdk_tools="${BDK_TOOLS_PATH:-/home/osterloh/brillo/tools/bdk}"
  fd=$((`cd /dev/fd && printf -- '%s
' * | sort -n | tail -1` + 1))
  eval 'exec '$fd'<<EOFEOF
  $(${bdk_tools}/cli/bdk product envsetup \
